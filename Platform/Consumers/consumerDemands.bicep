...
import { ConsumerModelAccess, MappedConsumerDemand, ConsumerDemandOutput, ConsumerNameToClientIdMapping } from '../types.bicep'
...
module consumer './consumerDemand.bicep' = [
  for idx in range(0, length(mappedDemands)): {
    ...
    params: {
      ...
      consumerAppId: filter(consumerNameToClientIdMappings, item => item.consumerName == consumerDemands[idx].consumerName)[0].entraClientId
    }
  }
]
